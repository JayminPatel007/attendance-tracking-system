...
**Nirdeshak** — owns the Sabhas of a single Sabha kind within a Kshetra (so each Kshetra has one Nirdeshak per Sabha kind: a Baal Nirdeshak, a Yuvak Nirdeshak, etc.). **Creates new Sabhas** within their (Kshetra, Sabha kind) scope.
_Avoid_: Director

**Sah-Nirdeshak** — co-Nirdeshak for the same (Kshetra, Sabha kind) scope.

**Sanyojak** — owns the Sabhas of a single Sabha kind within a Zone (so each Zone has one Sanyojak per Sabha kind). **Creates new Kshetras** within their Zone.
...
**Regional Team** _(canonical domain name TBD)_ — a City-level oversight body, scoped per **City × Sabha kind** (mirroring Sant's scope shape). A **body of multiple Users**, each with their own login, all carrying the "Regional Team member of (City X, Sabha-kind Y)" role. Not a Karyakar role. **Appoints Sanyojaks** within their (City, Sabha kind) scope and consumes analytics rolled up for that scope. Members of the Regional Team are added by Madhyastha Karyalaya. Does *not* create Zones, Cities, or Sabha Kinds — those remain Madhyastha Karyalaya authority per ADR-0009.

**Madhyastha Karyalaya** — the state-level oversight body. **Exactly one per State, unified across all Sabha kinds.** A **body of multiple Users**, each with their own individual login; each member carries the "Madhyastha Karyalaya member" role assignment for the State. Not a Karyakar role. **Creates new Cities, Zones, and Sabha Kinds**, with each creation attributed to the specific member User who performed it. **Appoints Regional Team members and creates Sant User records** as administrative acts. The first MK member is seeded at install time (one-off bootstrap); subsequent members are added by any existing MK member.
