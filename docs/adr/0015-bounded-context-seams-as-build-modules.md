...
**Status**: superseded by [ADR-0019](0019-bounded-context-module-taxonomy.md). **Originally superseded [ADR-0008](0008-single-bounded-context-with-internal-seams.md)**; amended by [ADR-0017](0017-rest-adapters-live-in-application-modules.md), extended by [ADR-0018](0018-application-service-split.md), then fully superseded by ADR-0019. The 3-module-per-context layout (`*-domain` / `*-application` / `*-infrastructure`) and the `bootstrap` / `shared-kernel` names this ADR introduced no longer apply — see ADR-0019 for the current 5-module layout and renamed `application-container` / `common-domain`. The substantive claim that the system is one bounded context with compile-enforced internal seams survives in ADR-0019; what changed is the *shape* of those seams.
