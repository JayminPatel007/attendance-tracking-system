...
**Status**: accepted (issue #79). Records a rule the codebase already mostly follows and consolidates the last clear offenders. Feeds the #68 design spike ([ADR-0027](0027-no-shared-granted-scope-module-behind-the-authorization-engines.md)). **First enforced by [ADR-0032](0032-caller-authority-resolved-at-the-request-edge.md)** (rule R1: outside `identity-data-access`, `role_assignments` may appear only in an allowlisted file, each entry carrying its clause-1 justification — a source-text check, not ArchUnit, because the SQL lives in text blocks bytecode does not expose). The enforcing rule is `RoleAssignmentsAccessRuleTest` in `apps/backend/application-container`; its allowlist is where clause 1's examples are now kept honest, and its three failure modes mean an entry that goes stale fails the build rather than accumulating. The rule below is unchanged; it now has a test.
