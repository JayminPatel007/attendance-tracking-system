...
- **`CallerResolver` narrows to one method.** `resolveUserId` stays in `common-domain`, with exactly two consumers left and no application service among them: the argument resolver, and `LoginActivityListener`, which is event-driven rather than request-bound and so genuinely holds a subject. `requireUserId` — issue #78's single home for the `orElseThrow` every application service was repeating — is **deleted**: once no application service calls it, its home *is* the edge, and the resolver does the `orElseThrow` itself. `CallerUnknownException` stays in `common-domain` (issue #78 moved it there to fix identity's 500) and still maps to 403.
- **An ArchUnit rule keeps it that way.** `only_the_edge_knows_what_a_credential_is` forbids any dependency on `org.springframework.security.core..` / `.oauth2..` outside `common-application` and `application-container`. It has teeth — those jars are on every `*-application` module's classpath, so a re-introduced `UUID.fromString(jwt.getSubject())` compiles fine. It also forecloses reading any *other* claim in a controller; no endpoint needs one today, and if one does, the honest move is to widen what the edge resolves.
...
- **`/api/whoami` changes port, and one dies.** It used to answer by looking the subject up itself (`UserRepository.findByKeycloakUserId`); with the caller already resolved it reads `findById`. That was the method's last production caller, so it is **deleted** from the port and its JDBC adapter — the subject→User lookup now exists in exactly one place, `CallerResolver`, which is the whole point.
